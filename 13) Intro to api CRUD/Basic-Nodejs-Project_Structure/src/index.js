const { ServerConfig, Logger } = require("./config"); // If u are requiring from index.js file u don't need to 
...
app.use("/api", apiRoutes); // import apiRoutes from the ./routes folder & whenever somebody gives me an URL that starts with /api 
//I will redirect all the requests to the apiRoutes. | |  Link : http://localhost:3000/api
...
We know how to use middlewares inside the app.get() / app.post(),
 but instead of adding the middlewares over there we will register/add the middlewares inside the app.use().
*/
